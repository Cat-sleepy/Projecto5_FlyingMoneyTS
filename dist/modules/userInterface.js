...
export function entrarModoEdicao(id) {
    editingId = id;
}
/** Preenche o formulário com uma transação existente (modo edição). */
export function preencherFormularioEdicao(transacao) {
    if (!refs.inputDescricao || !refs.inputValor || !refs.selectTipo || !refs.root || !refs.form)
        return;
    ...
    if (txt)
        txt.textContent = "Guardar alterações";
}
export function sairModoEdicao() {
    ...
    const txt = refs.form.querySelector(".btn-text");
    if (txt)
        txt.textContent = "Adicionar ao histórico";
}
...
export function criarEstrutura(rootSelector = "#app") {
    ...
    if (!root)
        throw new Error(`UI: não encontrei o root ${rootSelector}`);
    refs.root = root;
    ...
    const tipo = el("div", { className: "tipo-transacao-container" }, [
        ...
        (() => {
            ...
            s.append(el("option", { value: "receita", textContent: "Receita" }), el("option", { value: "despesa", textContent: "Despesa" }));
            return s;
        })(),
    ]);
    ...
    refs.root.addEventListener("click", (e) => {
        ...
        if (!target)
            return;
        const edit = target.closest("[data-action='edit']");
        if (edit) {
            const id = edit.dataset.id;
            if (id && typeof onEditHandler === "function")
                onEditHandler(id);
            return;
        }
        ...
        if (del) {
            ...
            if (id && typeof onRemoveHandler === "function")
                onRemoveHandler(id);
        }
    });
    // Guarda refs
    ...
    root.addEventListener("click", (e) => {
        ...
        selectedCategoria = (_a = btn.dataset.value) !== null && _a !== void 0 ? _a : null;
    });
...
export function lerFormulario() {
    var _a, _b, _c;
    const descricao = (((_a = refs.inputDescricao) === null || _a === void 0 ? void 0 : _a.value) || "").trim();
    const valorRaw = (_b = refs.inputValor) === null || _b === void 0 ? void 0 : _b.value;
    const valor = Number(valorRaw);
    const tipoRaw = (_c = refs.selectTipo) === null || _c === void 0 ? void 0 : _c.value;
    if (!descricao)
        throw new Error("Preenche a descrição.");
    if (!Number.isFinite(valor) || valor <= 0)
        throw new Error("O valor tem de ser um número positivo.");
    if (tipoRaw !== "receita" && tipoRaw !== "despesa")
        throw new Error("Tipo inválido.");
    if (!selectedCategoria)
        throw new Error("Selecciona uma categoria.");
    return {
        descricao,
        valor,
        tipo: tipoRaw,
...
export function atualizarCards({ saldo, receitas, despesas }) {
    if (refs.cardSaldo)
        refs.cardSaldo.textContent = formatarMoeda(saldo);
    if (refs.cardReceitas)
        refs.cardReceitas.textContent = formatarMoeda(receitas);
    if (refs.cardDespesas)
        refs.cardDespesas.textContent = formatarMoeda(despesas);
}
